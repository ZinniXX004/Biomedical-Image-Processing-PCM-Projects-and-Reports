#!/usr/bin/env python3
"""
nuclei_seg_gui.py  — MoNuSeg 2018 Nuclei Segmentation GUI
PyQt6 + Matplotlib · Dark-Blue Theme · Version 2.1
...
# Matplotlib background colours (matching palette)
MPL_STYLE = {
...
DARK_QSS = """
QMainWindow, QWidget { background-color: %(bg1)s; color: %(text0)s;
  font-family: "Segoe UI","SF Pro Display","Helvetica Neue",Arial,sans-serif;
  font-size: 13px; }
QScrollArea { border:none; background:transparent; }
QScrollBar:vertical { background:%(bg2)s; width:8px; border-radius:4px; }
...
QScrollBar:horizontal { background:%(bg2)s; height:8px; border-radius:4px; }
...
QGroupBox { border:1px solid %(border2)s; border-radius:6px; margin-top:10px;
  padding-top:8px; background:%(bg2)s; }
QGroupBox::title { subcontrol-origin:margin; subcontrol-position:top left;
  left:10px; padding:0 6px; color:%(accent2)s; font-weight:600; font-size:12px; }
QTabWidget::pane { background:%(bg2)s; border:1px solid %(border)s;
  border-radius:0 6px 6px 6px; }
QTabBar::tab { background:%(bg3)s; color:%(text2)s; border:1px solid %(border2)s;
  border-bottom:none; padding:7px 14px; border-radius:4px 4px 0 0; margin-right:2px;
  font-size:12px; font-weight:500; }
QTabBar::tab:selected { background:%(tab_active)s; color:%(text0)s;
  border-color:%(accent1)s; font-weight:700; }
...
QLineEdit, QTextEdit { background:%(bg4)s; color:%(text0)s; border:1px solid %(border)s;
  border-radius:4px; padding:4px 8px; selection-background-color:%(accent1)s; }
QLineEdit:focus, QTextEdit:focus { border-color:%(accent2)s; }
QSpinBox, QDoubleSpinBox { background:%(bg4)s; color:%(text0)s; border:1px solid %(border)s;
  border-radius:4px; padding:3px 5px; }
QSpinBox:focus,QDoubleSpinBox:focus { border-color:%(accent2)s; }
QSpinBox::up-button,QDoubleSpinBox::up-button,
QSpinBox::down-button,QDoubleSpinBox::down-button { width:14px; background:%(bg3)s; border:none; }
QComboBox { background:%(bg4)s; color:%(text0)s; border:1px solid %(border)s;
  border-radius:4px; padding:4px 8px; }
QComboBox:focus { border-color:%(accent2)s; }
QComboBox::drop-down { border:none; width:20px; }
QComboBox QAbstractItemView { background:%(bg3)s; color:%(text0)s;
  border:1px solid %(border)s; selection-background-color:%(accent1)s; }
QCheckBox { color:%(text1)s; spacing:6px; }
QCheckBox::indicator { width:15px; height:15px; border:1px solid %(border)s;
  border-radius:3px; background:%(bg4)s; }
QCheckBox::indicator:checked { background:%(accent1)s; border-color:%(accent2)s; }
QCheckBox::indicator:hover { border-color:%(accent2)s; }
QPushButton { background:%(bg3)s; color:%(text0)s; border:1px solid %(border)s;
...
QPushButton#btnRun { background:qlineargradient(x1:0,y1:0,x2:0,y2:1,
  stop:0 #1B6F24,stop:1 #0F5217); color:#CCFFCC;
  border:1px solid #2E7D32; font-weight:700; font-size:13px; padding:10px 20px; }
QPushButton#btnRun:hover { background:qlineargradient(x1:0,y1:0,x2:0,y2:1,
  stop:0 #2E7D32,stop:1 #1B5E20); }
QPushButton#btnRun:disabled { background:%(bg3)s; color:%(text3)s; border-color:%(border2)s; }
QPushButton#btnSave { background:%(btn_save)s; color:%(accent2)s;
  border:1px solid %(accent1)s; font-weight:600; }
QPushButton#btnSave:hover { background:%(btn_sh)s; }
QPushButton#btnBrowse { background:%(bg3)s; color:%(text1)s;
  border:1px solid %(border)s; padding:4px 10px; font-size:11px; }
QPushButton#btnBrowse:hover { border-color:%(accent2)s; color:%(text0)s; }
QProgressBar { background:%(bg3)s; border:1px solid %(border)s; border-radius:3px;
  height:5px; color:transparent; }
QProgressBar::chunk { background:qlineargradient(x1:0,y1:0,x2:1,y2:0,
  stop:0 %(accent1)s,stop:1 %(accent4)s); border-radius:3px; }
QListWidget { background:%(bg4)s; border:1px solid %(border)s; border-radius:4px; }
...
class MatplotlibCanvas(QWidget):
    ...
    def update_figure(self, fig: Figure):
        ...
        old.addWidget(self._toolbar)

        self._scroll.setWidget(self._canvas)
        old.addWidget(self._scroll)

        # Adjust canvas size to figure
...
class SidebarSection(QGroupBox):
    """Collapsible group box for sidebar sections."""
...
class NucleiSegApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self._bg_label = QLabel(self._central)
        ...
        sidebar_scroll.setStyleSheet(
            f"background:{PAL['sidebar']}; border:none;"
            f"border-right:1px solid {PAL['border']};")
        sidebar_inner = QWidget()
        sidebar_inner.setStyleSheet(f"background:{PAL['sidebar']};")
        ...
        self._tabs.setStyleSheet("QTabWidget{margin:0;}")
    ...
    def _build_sidebar(self):
        ...
        logo.setStyleSheet(
            ...
            f"padding:10px 4px 6px 4px; border-bottom:1px solid {PAL['border']};")
        logo.setAlignment(Qt.AlignmentFlag.AlignCenter)
        sl.addWidget(logo)

        sub = QLabel("MoNuSeg 2018  ·  Pipeline v2.1")
        sub.setStyleSheet(f"color:{PAL['text2']}; font-size:11px; padding:0 0 6px 0;")
    ...
    def _build_main_tabs(self):
        ...
        hdr = QLabel(
            ...
            "<span style='font-size:12px;color:#6B8FBD;'>Pipeline v2.1 · PyQt6 GUI · ITS Teknik Biomedik</span>")
        ...
        pipe_lbl = QLabel(
            ...
            "① H&E Stain Extraction → ② CLAHE Enhancement → ③ Gaussian Blur<br>"
            "→ ④ Adaptive Threshold → ⑤ Morphological Open → ⑥ Close + Hole Fill<br>"
            "→ ⑦ (Optional) Watershed → ⑧ Size Filter → IoU · Dice · Time"
            "</span>")
    ...
    def _update_bg(self):
        """Draw background: image or dark gradient fallback."""
        ...
        if self._bg_pixmap:
            ...
            # Dark overlay
            painter.fillRect(0, 0, w, h, QColor(5, 13, 26, 220))
        else:
            # Dark-blue gradient fallback
