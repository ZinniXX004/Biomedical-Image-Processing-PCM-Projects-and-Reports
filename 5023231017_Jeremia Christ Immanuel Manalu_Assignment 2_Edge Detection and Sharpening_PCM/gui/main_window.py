...
from core.pipeline import compute_full_pipeline, compute_all_methods
...
class MainWindow(QMainWindow):
    ...
    def _create_card(self, title: str, accent_color: str) -> tuple[QWidget, QVBoxLayout]:
        ...
        card.setStyleSheet(f"""
            #ModernCard {{ background-color: {BG_CARD}; border: 1px solid {GRID_COL}; border-radius: 10px; }}
        """)
    ...
    # ─── TAB 1 TO 5 BUILDERS (ASSIGNMENT 1) ──────────────────────────────────
    ...
    # ─── TAB 6 TO 8 BUILDERS (ASSIGNMENT 2) ──────────────────────────────────
    ...
    def _sigma_changed(self, val):
        self.sigma_lbl.setText(f"σ = {val / 10.0:.2f}")
        self._update()

    def _thr_changed(self, val):
        self.thr_lbl.setText(f"threshold = {val / 100.0:.2f}")
        self._update()
    ...
    def _do_update(self):
        ...
        res = compute_full_pipeline(
            self.gray_img, method, sigma, thr, enh_m,
            self.enh_clahe_clip, self.cs_low, self.cs_high
        )
    ...
    def _do_update_enhancement(self):
        ...
        enhs = compute_all_enhancements(
            acq, clahe_clip=self.enh_clahe_clip, cs_low=self.cs_low, cs_high=self.cs_high
        )
    ...
    def _do_update_runtime(self):
        ...
        try:
            ...
            self.analysis_panels["edge_compare_grid"].show_image_grid(
                edge_imgs, title="Edge Detection Comparison — All Methods", cmap="gray", cols=4)
            ...
            self.analysis_panels["sharp_compare_grid"].show_image_grid(
                sharp_imgs, title="Sharpening Comparison — Original vs Laplacian vs USM", cmap="gray", cols=3)
