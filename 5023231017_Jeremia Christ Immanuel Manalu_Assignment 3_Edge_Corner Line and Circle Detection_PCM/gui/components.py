...
class AnalysisPanel(QWidget):
    ...
    def show_performance_eval(self, metrics_per_method: dict) -> None:
        self.fig.clear()
        metric_cfg =[("RMSE", "↓ lower = better", "#F38BA8"), ("PSNR", "↑ higher = better[dB]", "#A6E3A1"),
                     ("SSIM", "↑ higher = better  [0–1]", "#89B4FA"), ("Entropy", "↑ higher = info[bits]", "#CBA6F7")]
        ...
        for idx, (mname, hint, accent) in enumerate(metric_cfg):
            ax = self.fig.add_subplot(2, 2, idx + 1); ax.set_facecolor(BG_PANEL)
            ...
            if bars is not None:
                iterable_bars = bars.patches if hasattr(bars, 'patches') else bars
                for bar, val in zip(iterable_bars, vals):
            ...
            ax.set_xticks(range(len(methods))); ax.set_xticklabels(methods, fontsize=9, color=TEXT_COL, fontweight="bold")
            ax.grid(True, alpha=0.15, color=SUBTEXT, axis="y")
            _mn = min(vals); _mx = max(vals)
            ax.set_ylim(max(0.0, _mn * 0.88), _mx * 1.18 if _mx > 0 else 1.0)
            
        self.fig.suptitle("Enhancement Performance Evaluation", fontsize=11, color=TEXT_COL, y=0.98)
    ...
    def show_runtime_bars(self, runtimes_dict: dict) -> None:
        ...
        bars = ax.barh(methods, times, color=bar_colors, alpha=0.85, edgecolor=GRID_COL, linewidth=0.8, height=0.55)
        
        if bars is not None:
            iterable_bars = bars.patches if hasattr(bars, 'patches') else bars
            for bar, t in zip(iterable_bars, times):
        ...
        ax.set_title("Method Runtime Comparison", fontsize=11, color=TEXT_COL, pad=8)
        ax.grid(True, alpha=0.15, color=SUBTEXT, axis="x"); ax.set_xlim(0, t_max * 1.32)
        self.fig.subplots_adjust(left=0.25, right=0.88, top=0.90, bottom=0.12)
        self.canvas.draw_idle()
